...
$subject = isset($_POST["widget-contact-form-subject"]) ? $_POST["widget-contact-form-subject"] : 'MENSAJE DESDE EL SITIO WEB EMPREC';
...
if( $_SERVER['REQUEST_METHOD'] == 'POST') {
 ...
 if($email != '') {
                ...
                $mail->Body = $name . $email . $phone . $company . $service . $message . '<br><br><br>Este email fue enviado por: ' . $_SERVER['HTTP_REFERER'];
            ...
            if(isset($recaptcha) && $recaptcha == '') {
             $recaptcha_response=json_decode(file_get_contents("https://www.google.com/recaptcha/api/siteverify?secret=&response=".$recaptcha."&remoteip=".$_SERVER['REMOTE_ADDR']), true);
                        ...
                        {
                            $response = array ('response'=>'error', 'message'=> "INTENTE DE NUEVO");
                        }
