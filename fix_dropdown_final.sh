...
# Script to fix dropdown JavaScript in all HTML files - Final version
...
for file in $files_to_update; do
    ...
    # Fix the dropdown-toggle click event - remove the incorrect condition
    sed -i '' 's/if ($(window).width() <= 991 && !$(this).hasClass('\''dropdown-toggle'\'')) {/if ($(window).width() <= 991) {/g' "$file"
    
    # Fix the dropdown click event - remove the incorrect condition  
    sed -i '' 's/if ($(window).width() <= 991 && !$(this).hasClass('\''dropdown-toggle'\'')) {/if ($(window).width() <= 991) {/g' "$file"
    
    # Fix the click outside event - remove the incorrect condition
    sed -i '' 's/if ($(window).width() <= 991 && !$(this).hasClass('\''dropdown-toggle'\'')) {/if ($(window).width() <= 991) {/g' "$file"
